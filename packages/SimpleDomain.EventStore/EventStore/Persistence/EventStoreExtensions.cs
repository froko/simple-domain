...
{
    public static byte[] AsByteArray(this IEvent @event) => Serialize(@event);
    ...
    public static IAsyncEnumerable<IEvent> DeserializeAsEvents(this EventStoreClient.ReadStreamResult stream, int skip)
    ...
    public static IEvent DeserializeAsEvent(this ResolvedEvent resolvedEvent)
    {
        var eventData = resolvedEvent.Event.Data;
        var typeName = resolvedEvent.Event.EventType;
        return eventData.Deserialize<IEvent>(typeName);
    }
