...
public class SnapshotStrategy
{
    ...
    private bool IsAssignableToMe<TAggregateRoot>() where TAggregateRoot : EventSourcedAggregateRoot =>
        this.aggregateType.IsAssignableFrom(typeof(TAggregateRoot));

    private bool IamNotTheAggregateRootBaseType() => this.aggregateType != typeof(EventSourcedAggregateRoot);

    private bool IsMultipleOfThreshold(int version) => version != 0 && version % this.threshold == 0;
}
