...
public class EventStoreRepositoryTests
{
    ...
    public EventStoreRepositoryTests()
    {
        A.CallTo(() => this.eventStore
                .OpenStream<MyDynamicEventSourcedAggregateRoot>(AggregateId, default))
            .Returns(this.eventStream);

        A.CallTo(() => this.configuration
                .GetSnapshotStrategy<MyDynamicEventSourcedAggregateRoot>())
    ...
    {
        A.CallTo(() => this.configuration.GetSnapshotStrategy<MyDynamicEventSourcedAggregateRoot>())
            .Returns(new SnapshotStrategy(10));

        var aggregateRoot = new MyDynamicEventSourcedAggregateRoot(AggregateId).WithVersion(10);
