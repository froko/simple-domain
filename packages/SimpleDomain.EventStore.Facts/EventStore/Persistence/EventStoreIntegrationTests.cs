...
public class EventStoreIntegrationTests : IAsyncLifetime
{
    ...
    private static string DynamicAggregateRootName => $"{nameof(MyDynamicEventSourcedAggregateRoot)}-{AggregateId}";
    private static string StaticAggregateRootName => $"{nameof(MyStaticEventSourcedAggregateRoot)}-{AggregateId}";

    private static string StaticAggregateRootSnapshotName =>
        $"{nameof(MyStaticEventSourcedAggregateRoot)}-{AggregateId}-Snapshot";
    ...
    public async Task InitializeAsync()
    {
        ...
        await DeleteEventStream(client, DynamicAggregateRootName);
        await DeleteEventStream(client, StaticAggregateRootName);
        await DeleteEventStream(client, StaticAggregateRootSnapshotName);
    }
    ...
    {
        var aggregateRoot = new MyStaticEventSourcedAggregateRoot(AggregateId);
        aggregateRoot.ChangeValue(0);
        aggregateRoot.ChangeValue(11);
        aggregateRoot.ChangeValue(22);

        using var eventStream = await CreateEventStream<MyStaticEventSourcedAggregateRoot>();
    ...
    public async Task AppendsAndReplaysEventsFromLatestSnapshot()
    {
        ...
        await AppendEvents<MyStaticEventSourcedAggregateRoot>(aggregateRoot);

        var firstSnapshot = aggregateRoot.CreateSnapshot();
        await SaveSnapshot<MyStaticEventSourcedAggregateRoot>(firstSnapshot);
        ...
        await SaveSnapshot<MyStaticEventSourcedAggregateRoot>(secondSnapshot);
    ...
    public async Task ReplaysAllEventsInOrderOfOccurrence()
    {
        ...
        await AppendEvents<MyDynamicEventSourcedAggregateRoot>(dynamicAggregateRoot);
        await AppendEvents<MyStaticEventSourcedAggregateRoot>(staticAggregateRoot);
        ...
        A.CallTo(() => eventPublisher.Invoke(A<ValueEvent>.That.Matches(v => v.Value == 11), A<CancellationToken>._))
            .MustHaveHappened()
            .Then(
                A.CallTo(
                        () =>
                            eventPublisher.Invoke(
                                A<ValueEvent>.That.Matches(v => v.Value == 22),
            ...
            .Then(
                A.CallTo(
                        () =>
                            eventPublisher.Invoke(
                                A<ValueEvent>.That.Matches(v => v.Value == 111),
                                A<CancellationToken>._
                            )
                    )
                    .MustHaveHappened()
            ...
            .Then(
                A.CallTo(
                        () =>
                            eventPublisher.Invoke(
                                A<ValueEvent>.That.Matches(v => v.Value == 33),
                                A<CancellationToken>._
                            )
                    )
                    .MustHaveHappened()
            ...
            .Then(
                A.CallTo(
                        () =>
                            eventPublisher.Invoke(
                                A<ValueEvent>.That.Matches(v => v.Value == 222),
                                A<CancellationToken>._
                            )
                    )
                    .MustHaveHappened()
            ...
            .Then(
                A.CallTo(
                        () =>
                            eventPublisher.Invoke(
                                A<ValueEvent>.That.Matches(v => v.Value == 333),
                                A<CancellationToken>._
                            )
                    )
                    .MustHaveHappened()
